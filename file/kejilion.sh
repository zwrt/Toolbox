...
kpanel_hosts_noninteractive() {
	[ "${KJ_HOSTS_NONINTERACTIVE:-}" = "1" ] || return 2
	[ "$EUID" -eq 0 ] || {
		echo "错误: KPanel hosts 协议必须以 root 运行"
		return 1
	}
	[ "$#" -eq 2 ] || {
		echo "错误: hosts 协议需要操作和一条解析记录"
		return 1
	}

	local operation="$1" entry="$2" value normalized=""
	local parts=()
	read -r -a parts <<< "$entry"
	[ "${#parts[@]}" -ge 2 ] && [ "${#parts[@]}" -le 16 ] || {
		echo "错误: hosts 记录格式必须为 IP 后跟 1-15 个主机名"
		return 1
	}
	if ! kpanel_dns_is_ipv4 "${parts[0]}" && ! kpanel_dns_is_ipv6 "${parts[0]}"; then
		echo "错误: hosts 记录必须以有效 IP 地址开头"
		return 1
	fi
	normalized="${parts[0]}"
	for value in "${parts[@]:1}"; do
		[[ "$value" =~ ^[A-Za-z0-9_.-]{1,253}$ ]] || {
			echo "错误: hosts 主机名无效"
			return 1
		}
		normalized+=" $value"
	...
	local hosts_file="/etc/hosts" backup desired
	backup="$(mktemp /etc/.hosts.kpanel.backup.XXXXXX)" || return 1
	desired="$(mktemp /etc/.hosts.kpanel.desired.XXXXXX)" || {
		rm -f "$backup"
		return 1
	}
	...
	case "$operation" in
		add)
			if awk -v target="$normalized" '
				/^[[:space:]]*#/ || /^[[:space:]]*$/ { next }
				{ line=$1; for (i=2; i<=NF; i++) line=line " " $i; if (line==target) found=1 }
				END { exit found ? 0 : 1 }
			' "$hosts_file"; then
				rm -f "$backup" "$desired"
				echo "KPANEL_HOSTS_RESULT unchanged"
				return 0
			fi
			cp -p "$hosts_file" "$desired" && printf '%s\n' "$normalized" >> "$desired"
			;;
		delete)
			awk -v target="$normalized" '
				{
					line=$1; for (i=2; i<=NF; i++) line=line " " $i
					if ($0 !~ /^[[:space:]]*#/ && $0 !~ /^[[:space:]]*$/ && line==target) { found=1; next }
					print
				}
				END { if (!found) exit 3 }
			' "$hosts_file" > "$desired"
		...
		*)
			rm -f "$backup" "$desired"
			echo "错误: hosts 操作必须为 add 或 delete"
			return 1
			;;
	esac

	chmod --reference="$hosts_file" "$desired" 2>/dev/null || chmod 644 "$desired"
	chown --reference="$hosts_file" "$desired" 2>/dev/null || true
	if ! mv -f "$desired" "$hosts_file"; then
		cp -p "$backup" "$hosts_file" >/dev/null 2>&1 || true
		rm -f "$backup" "$desired"
		echo "错误: hosts 写入失败，已尝试恢复原文件"
		return 1
	fi
	rm -f "$backup"
	echo "KPANEL_HOSTS_RESULT applied"
}

kpanel_cron_noninteractive() {
	[ "${KJ_CRON_NONINTERACTIVE:-}" = "1" ] || return 2
	[ "$EUID" -eq 0 ] || {
		echo "错误: KPanel cron 协议必须以 root 运行"
		return 1
	}
	[ "$#" -eq 2 ] || {
		echo "错误: cron 协议需要操作和一条任务记录"
		return 1
	}

	local operation="$1" entry="$2" field
	[ -n "$entry" ] && [ ${#entry} -le 4096 ] && [[ "$entry" != *$'\n'* ]] && [[ "$entry" != *$'\r'* ]] || {
		echo "错误: cron 任务为空、过长或包含换行"
		return 1
	}
	if [ "$operation" = "add" ]; then
		...
		read -r -a parts <<< "$entry"
		[ "${#parts[@]}" -ge 6 ] || {
			echo "错误: cron 任务必须包含五段时间表达式和执行命令"
			return 1
		}
		for field in "${parts[@]:0:5}"; do
			[[ "$field" =~ ^[0-9*/?,\-]+$ ]] || {
				echo "错误: cron 时间表达式仅允许数字及 */?, -"
				return 1
			}
		done
	elif [ "$operation" != "delete" ]; then
		echo "错误: cron 操作必须为 add 或 delete"
		return 1
	fi

	local current desired
	current="$(mktemp /tmp/kpanel-cron-current.XXXXXX)" || return 1
	desired="$(mktemp /tmp/kpanel-cron-desired.XXXXXX)" || {
		rm -f "$current"
		return 1
	}
	...
	case "$operation" in
		add)
			if grep -Fxq -- "$entry" "$current"; then
				...
				echo "KPANEL_CRON_RESULT unchanged"
				return 0
			fi
			cp "$current" "$desired" && printf '%s\n' "$entry" >> "$desired"
			;;
		delete)
			if ! grep -Fxq -- "$entry" "$current"; then
				rm -f "$current" "$desired"
			...
			awk -v target="$entry" '$0 != target { print }' "$current" > "$desired"
			;;
	esac
	if ! crontab "$desired"; then
		crontab "$current" >/dev/null 2>&1 || true
		rm -f "$current" "$desired"
		echo "错误: cron 写入失败，已尝试恢复原任务"
		return 1
	fi
	rm -f "$current" "$desired"
	echo "KPANEL_CRON_RESULT applied"
}

kpanel_network_noninteractive() {
	[ "${KJ_NETWORK_NONINTERACTIVE:-}" = "1" ] || return 2
	[ "$EUID" -eq 0 ] || return 1
	[ "$#" -eq 2 ] || return 1
	local operation="$1" nic="$2"
	[[ "$nic" =~ ^[A-Za-z0-9_.:-]{1,64}$ ]] || return 1
	[ -e "/sys/class/net/$nic" ] || return 1
	case "$operation" in
		up|down) ip link set "$nic" "$operation" || return 1 ;;
		*) return 1 ;;
	esac
	local link_state
	link_state="$(ip link show "$nic" 2>/dev/null | head -n1)"
	if { [ "$operation" = "up" ] && [[ "$link_state" != *"UP"* ]]; } ||
		{ [ "$operation" = "down" ] && [[ "$link_state" == *"UP"* ]]; }; then
		echo "错误: 网卡状态回读验证失败"
		return 1
	fi
	echo "KPANEL_NETWORK_RESULT applied"
}

kpanel_firewall_noninteractive() {
	[ "${KJ_FIREWALL_NONINTERACTIVE:-}" = "1" ] || return 2
	[ "$EUID" -eq 0 ] || return 1
	[ "$#" -ge 1 ] || return 1
	local operation="$1" value="${2:-}" code current_port rc=0 snapshot had_ipset=false
	snapshot="$(mktemp -d /tmp/kpanel-firewall.XXXXXX)" || return 1
	iptables-save > "$snapshot/iptables.v4" || { rm -rf -- "$snapshot"; return 1; }
	if command -v ip6tables-save >/dev/null 2>&1; then
		ip6tables-save > "$snapshot/iptables.v6" || { rm -rf -- "$snapshot"; return 1; }
	fi
	if command -v ipset >/dev/null 2>&1; then
		had_ipset=true
		ipset save > "$snapshot/ipset.rules" || { rm -rf -- "$snapshot"; return 1; }
	fi
	case "$operation" in
		port-open|port-close)
			[[ "$value" =~ ^[0-9]{1,5}$ ]] && [ "$value" -ge 1 ] && [ "$value" -le 65535 ] || rc=1
			if [ "$rc" -eq 0 ] && [ "$operation" = "port-open" ]; then open_port "$value" || rc=1
			elif [ "$rc" -eq 0 ]; then close_port "$value" || rc=1; fi
			if [ "$rc" -eq 0 ] && [ "$operation" = "port-open" ]; then
				iptables -C INPUT -p tcp --dport "$value" -j ACCEPT >/dev/null 2>&1 && iptables -C INPUT -p udp --dport "$value" -j ACCEPT >/dev/null 2>&1 || rc=1
			elif [ "$rc" -eq 0 ]; then
				iptables -C INPUT -p tcp --dport "$value" -j DROP >/dev/null 2>&1 && iptables -C INPUT -p udp --dport "$value" -j DROP >/dev/null 2>&1 || rc=1
			fi
			;;
		ip-allow|ip-block|ip-remove)
			[[ "$value" =~ ^[0-9A-Fa-f:.]+(/[0-9]{1,3})?$ ]] || rc=1
			case "$operation:$rc" in
				ip-allow:0) allow_ip "$value" || rc=1; iptables -C INPUT -s "$value" -j ACCEPT >/dev/null 2>&1 || rc=1 ;;
				ip-block:0) block_ip "$value" || rc=1; iptables -C INPUT -s "$value" -j DROP >/dev/null 2>&1 || rc=1 ;;
				ip-remove:0)
					iptables -D INPUT -s "$value" -j ACCEPT 2>/dev/null || true
					iptables -D INPUT -s "$value" -j DROP 2>/dev/null || true
					iptables -C INPUT -s "$value" -j ACCEPT >/dev/null 2>&1 && rc=1
					iptables -C INPUT -s "$value" -j DROP >/dev/null 2>&1 && rc=1
					;;
			esac
			;;
		ping-allow)
			iptables -C INPUT -p icmp --icmp-type echo-request -j ACCEPT 2>/dev/null || iptables -A INPUT -p icmp --icmp-type echo-request -j ACCEPT || rc=1
			iptables -C INPUT -p icmp --icmp-type echo-request -j ACCEPT >/dev/null 2>&1 || rc=1
			;;
		ping-block)
			iptables -D INPUT -p icmp --icmp-type echo-request -j ACCEPT 2>/dev/null || true
			iptables -C INPUT -p icmp --icmp-type echo-request -j ACCEPT >/dev/null 2>&1 && rc=1
			;;
		ddos-enable) enable_ddos_defense || rc=1; iptables -C INPUT -p tcp --syn -j DROP >/dev/null 2>&1 || rc=1; iptables -C INPUT -p udp -j DROP >/dev/null 2>&1 || rc=1 ;;
		ddos-disable) disable_ddos_defense || rc=1; iptables -C INPUT -p tcp --syn -j DROP >/dev/null 2>&1 && rc=1; iptables -C INPUT -p udp -j DROP >/dev/null 2>&1 && rc=1 ;;
		country-block|country-allow|country-unblock)
			shift
			[ "$#" -ge 1 ] && [ "$#" -le 20 ] || rc=1
			for code in "$@"; do [[ "$code" =~ ^[A-Za-z]{2}$ ]] || rc=1; done
			[ "$rc" -eq 0 ] && manage_country_rules "${operation#country-}" "$@" || rc=1
			for code in "$@"; do
				local set_name="${code,,}_block"
				case "$operation" in
					country-block) ipset list "$set_name" >/dev/null 2>&1 && iptables -C INPUT -m set --match-set "$set_name" src -j DROP >/dev/null 2>&1 || rc=1 ;;
					country-allow) ipset list "$set_name" >/dev/null 2>&1 && iptables -C INPUT -m set --match-set "$set_name" src -j ACCEPT >/dev/null 2>&1 || rc=1 ;;
					country-unblock)
						iptables -D INPUT -m set --match-set "$set_name" src -j ACCEPT 2>/dev/null || true
						iptables -C INPUT -m set --match-set "$set_name" src -j DROP >/dev/null 2>&1 && rc=1
						iptables -C INPUT -m set --match-set "$set_name" src -j ACCEPT >/dev/null 2>&1 && rc=1
						;;
				esac
			done
			;;
		all-open|all-close)
			...
			iptables -F && iptables -X || rc=1
			if [ "$operation" = "all-open" ]; then
			...
			iptables -A INPUT -i lo -j ACCEPT
			iptables -A INPUT -p tcp --dport "$current_port" -j ACCEPT
			[ "$(iptables -S INPUT | sed -n 's/^-P INPUT //p')" = "$([ "$operation" = "all-open" ] && echo ACCEPT || echo DROP)" ] || rc=1
			iptables -C INPUT -p tcp --dport "$current_port" -j ACCEPT >/dev/null 2>&1 || rc=1
			;;
		*) rc=1 ;;
	esac
	if [ "$rc" -ne 0 ]; then
		iptables-restore < "$snapshot/iptables.v4" 2>/dev/null || true
		[ -s "$snapshot/iptables.v6" ] && ip6tables-restore < "$snapshot/iptables.v6" 2>/dev/null || true
		if [ "$had_ipset" = true ]; then
			case "$operation" in
				country-*)
					for code in "$@"; do
						local set_name="${code,,}_block"
						if grep -Fq "create $set_name " "$snapshot/ipset.rules"; then
			...
			[ -s "$snapshot/ipset.rules" ] && ipset restore -exist < "$snapshot/ipset.rules" 2>/dev/null || true
		fi
		rm -rf -- "$snapshot"
		echo "错误: 防火墙事务失败，已恢复原规则"
		return 1
	fi
	save_iptables_rules || { iptables-restore < "$snapshot/iptables.v4" 2>/dev/null || true; rm -rf -- "$snapshot"; return 1; }
	rm -rf -- "$snapshot"
	echo "KPANEL_FIREWALL_RESULT applied"
}
